---
name: vsg
description: Short, explicit alias for vs-router. Use only when the user explicitly invokes `use vsg:` and wants the shortest reliable router command.
---
...
- Treat non-explicit triggers (e.g., "vs go") as normal text; ask the user to rephrase using `use vsg:`.
...
- Before any file search, determine scope roots:
  1. If a `.vs-scope` file exists in the current directory or any parent, use the closest one.
     - Each non-empty, non-comment line is an allowed path.
     - Relative paths are resolved from the `.vs-scope` file directory.
...
- If the user did not explicitly invoke `use vsg:`, ask them to rephrase using `use vsg:` and stop.
