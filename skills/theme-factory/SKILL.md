...
- Auto-select a theme based on context if none is provided.
...
1. **Auto-pick for speed**: If the user has no preference or wants speed, select a theme based on context and apply it immediately.
2. **Show the theme showcase**: Otherwise, display the `theme-showcase.pdf` file to allow users to see all available themes visually. Do not make any modifications to it; simply show the file for viewing.
3. **Ask for their choice**: Ask which theme to apply to the deck.
4. **Wait for selection**: Get explicit confirmation about the chosen theme.
5. **Apply the theme**: Once a theme has been chosen, apply the selected theme's colors and fonts to the deck/artifact.
6. **Confirm or adjust**: After applying, ask for confirmation or theme changes.
